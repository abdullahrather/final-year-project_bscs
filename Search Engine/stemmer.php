	...
	class PorterStemmer
	{
		...
		*/
		...
		/** Test program for demonstrating the Stemmer.  It reads text from a
		  ...
		  */
		 public static function StemIt($webPage) {
			 //print_r($webPage);
			 //$s = new Stemmer();
		
			 //Step-1:  Open stop list, and set handle for it.
			 $buffer = ""; //For temporary storage of StopList.
			 $buffer = file_get_contents("StopList.txt");
			 $listWords = explode("\n", $buffer); //Words written in Ignore List
			 //print_r($listWords);
			 
			 ///Step-2:   Match Stop-List with words fetched from site
			 $keyWords;//Array to store stemmed Key Words.
			 $cWord;//Current Word
			 $allWords=""; //String to temporarily store all stemmed key words.
		
			 //This loop is to process all the words retrieved from webPage
			 for ($i = 0; $i < count($webPage); $i++) {
				 $cWord = trim($webPage[$i]); //using alias for webPage[i], i.e current word
				 
				 //If current word is a Tag or attribute etc, than don't see it in Stop-List
				 $isValidWord = true;
				 for ($k = 0; $k < strlen($cWord) && $isValidWord; $k++) {
					 //echo strlen($cWord)."-";
					 //$isValidWord = Character.isLetter($cWord.charAt($k));
					 //$isValidWord = ereg("[a-zA-Z]", $cWord{$k} );
					 $isValidWord = preg_match('/[a-zA-Z]/', $cWord{$k});
				 }
				 if (!$isValidWord)
				 ...
				 $initial = 0;
				 $mid = (int)(count($listWords) / 2);
				 $last = count($listWords);
				 do {
					 //if cWord is in starting side means listWords[i] is greater than  cWord
					 if (strcasecmp(trim($listWords[$mid]), $cWord) > 0)
						 $last = $mid - 1;
					 else //if cWord is in lower side
						 $initial = $mid + 1;
		
					 $mid = ($initial + $last) / 2; //setting mid
				 } while ((strcasecmp(trim($listWords[$mid]), $cWord) != 0) && $initial <= $last);
		
				 //If not found in list, than apply Stemming algo and put current word
				 // in the collection of keywords..
				 if (strcasecmp(trim($listWords[$mid]), $cWord) != 0) {
					 //echo "-".trim($listWords[$mid])."-".$cWord."-";
					 
					 //System.out.println(s.toString());
					 echo $cWord." -- ";
					 $stmdWord = self::stem($cWord);
					 echo $stmdWord."<br />";
					 
					 //if(strcmp($cWord, $stmdWord) != 0) {//If stemmed word is different then Origional
						 //System.out.println(cWord + " - " + s.toString());
					 $allWords = $allWords . "," . $stmdWord;
					 //}
				 }
				 //echo "found";
			 } //Ending for loop
